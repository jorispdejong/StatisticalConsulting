...
tvpSIR=function(country,esp,P,p,q){
  data=choose.country(country)
  dat=data$k
  date=dat[,1]
  N=data$N
  a=which(dat[,3]!=0)
  b=which(dat[,2]!=0)
  first.date=intersect(a,b)[1]  
...
choose.country=function(country){
  # if(Country=="Korea"){
  #   data_old = read.csv("https://raw.githubusercontent.com/ulklc/covid19-timeseries/master/countryReport/country/KR.csv")
  #   k=make.dat(data)
  #   N=51269185
  # }
  # if(Country=="Sweden"){
  ...
  if(country == "Afghanistan") data = global_covid_data$Afghanistan
  k_raw = data.frame('day'=str_replace_all(data$t, "[-]","/"), 
                 'confirmed'=data$confirmed,
                 'recovered'=data$recovered, 
                 'death'=data$deaths)
  # start from first confirmed case
  k = k_raw[which(k_raw$confirmed != 0)[1]:nrow(k_raw),]
  N = data$population
  return(list(k=k,N=N))
}
