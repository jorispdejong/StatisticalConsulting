...
### ESTIMATE R ###
##################
res <- est.R0.TD(epid = reported_cases$epid, 
                 t = reported_cases$t, 
                 GT = GT,
                 n.t0 = reported_cases$epid[1],
                 nsim = 100)
Rt <- res$R
Rt_lower <- res$conf.int$lower
Rt_upper <- res$conf.int$upper
Rt_t <- as.Date(names(Rt))
...
plot(Rt_t, Rt, type = 'l', col = cols[1],
     ...
     xlab = 't', ylab = 'R(t)',
     main = 'Effective Reproduction Number (R0 package)',
     xaxt='n', cex.lab=1.3)
...
legend('topright',
       legend = c('R(t) (R0)', '95% confidence interval (R0)', 'R(t) (RIVM)', '95% confidence interval (RIVM)'), 
